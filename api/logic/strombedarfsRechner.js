...
function calculateGesamtStrombedarfMonatlich(heizfensterTyp, gebaeudeStandard, gebaeudenutzflaeche, bewohnerAnzahl, twwBereitstellung) {
    let strombedarfHeizenProQm = calculateStrombedarfHeizenProQmMonatlich(heizfensterTyp, gebaeudeStandard);
    let strombedarfTwwProQm = calculateStrombedarfTwwProQmMonatlich(gebaeudenutzflaeche, twwBereitstellung);
    let strombedarfHaushaltProQm = calculateStrombedarfHaushaltProQmMonatlich(bewohnerAnzahl, gebaeudenutzflaeche);
    let gesamtSrombedarfMonatlich = [];
    for (let i = 0; i < 12; i++) {
        gesamtSrombedarfMonatlich[i] = calculateGesamtStrombedarfProQm(strombedarfHeizenProQm[i], strombedarfTwwProQm[i], strombedarfHaushaltProQm[i]) * gebaeudenutzflaeche;
    }
    return gesamtSrombedarfMonatlich;
}

// Strombedarf Gesamt (Heizen+Tww+Haushalt) Jährlich
function calculateGesamtStrombedarf(heizfensterTyp, gebaeudeStandard, gebaeudenutzflaeche, bewohnerAnzahl) {
    return sumAllArrayValues(calculateGesamtStrombedarfMonatlich(heizfensterTyp, gebaeudeStandard, gebaeudenutzflaeche, bewohnerAnzahl));
}

function calculateGesamtStrombedarfProQm(strombedarfHeizenProQm, strombedarfTwwProQm, strombedarfHaushaltProQm) {
    return strombedarfHeizenProQm + strombedarfTwwProQm + strombedarfHaushaltProQm;
}

//Strombedarf für Heizen pro QM Monatlich
function calculateStrombedarfHeizenProQmMonatlich(heizfensterTyp, gebaeudeStandard) {
    let strombedarfHeizenMonatlich = [];
    for (let i = 1; i <= 12; i++) {
        strombedarfHeizenMonatlich[i - 1] = calculateStrombedarfHeizenProQmProMonat(heizwaermebedarfRechner.calculateHeizwaermebedarfProQmProMonat(heizwaermebedarfRechner.calculateHeizwaermebedarfProQm(gebaeudeStandard), i), heizfensterTyp, i);
    }
...
function calculateStrombedarfHeizenProQm(heizfensterTyp, gebaeudeStandard) {
    return sumAllArrayValues(calculateStrombedarfHeizenProQmMonatlich(heizfensterTyp, gebaeudeStandard));
}

//Strombedarf für Heizen pro QM und Monat
function calculateStrombedarfHeizenProQmProMonat(heizwaermebedarfProQmProMonat, heizfensterTyp, monat) {
...
function calculateStrombedarfTwwProQmMonatlich(gebaeudenutzflaeche, twwBereitstellung) {
    let strombedarfTwwMonatlich = [];
    for (let i = 1; i <= 12; i++) {
        if (twwBereitstellung = "TWW-Wärmepumpe") {
            strombedarfTwwMonatlich[i - 1] = calculateStrombedarfTwwProQmProMonat(twwBedarfRechner.calculateTwwbedarfProQmProMonat(twwBedarfRechner.calculateTwwbedarfProQm(gebaeudenutzflaeche), i), i);
        } else {
            strombedarfTwwMonatlich[i - 1] = calculateStrombedarfDurchlaufProQmProMonat(twwBedarfRechner.calculateTwwbedarfProQmProMonat(twwBedarfRechner.calculateTwwbedarfProQm(gebaeudenutzflaeche), i), i)
        }
    }
...
function calculateStrombedarfTwwProQm(gebaeudenutzflaeche) {
    return sumAllArrayValues(calculateStrombedarfTwwProQmMonatlich(gebaeudenutzflaeche))
}
...
function calculateStrombedarfHaushaltProQmMonatlich(bewohnerAnzahl, gebaeudeNutzflaeche) {
    let stromBedarfHaushaltProQm = calculateStrombedarfHaushaltProQm(bewohnerAnzahl, gebaeudeNutzflaeche)
    let strombefarfHaushaltMonatlich = [];
    for (let i = 1; i <= 12; i++) {
        strombefarfHaushaltMonatlich[i - 1] = calculateStrombedarfHaushaltProQmProMonat(stromBedarfHaushaltProQm, i);
    }
...
function calculateStrombedarfHaushaltProQmProMonat(strombedarfHaushaltProQm, Monat) {
...
function calculateStrombedarfHaushaltProQm(anzahlBewohner, gebaeudenutzflaeche) {

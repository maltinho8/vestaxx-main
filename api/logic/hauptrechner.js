...
function calculateAllResults(input) {
  return {
    gesamtPrimaerEnergieBedarfHeizfensterProQm:
      calculateGesamtPrimaerEnergieBedarfHeizfensterProQm(input),
    gesamtPrimaerEnergieBedarfHeizfenster:
      calculateGesamtPrimaerEnergieBedarfHeizfenster(input),
    ...
    einspeiseVerguetungHeizfenster:
      calculateEinspeiseVerguetungHeizfenster(input),
    gesamtAnnuitaetHeizfensterOhneAnrechnung:
      calculateGesamtannuitaetHeizfenstersystemOhneAnrechnung(input),
    gesamtAnnuitaetHeizfensterMitAnrechnung:
      calculateGesamtannuitaetHeizfenstersystemMitAnrechnung(input),
    substituiertePrimaerEnergieHeizfensterProQM:
      calculateSubstituiertePrimaerEnergieHeizfensterProQM(input),
    co2Heizfenster: calculateCo2Heizfenster(input),
    co2SubstituiertHeizfenster:
      calculateSubstituierteCo2EmissionenHeizfenster(input),
    investitionErgasBwSystem: calculateInvestitionErdgasBwSystem(input),
    gesamtEnergiekostenErgasBwSystem:
      calculateGesamtEnergiekostenErdgasBwSystem(input),
    gesamtAnnuitaetErgasBwSystem: calculateGesamtannuitaetErdgasBwSystem(input),
    gesamtPrimaerenergiebedarfErdgasBwSystemProQm:
      calculateGesamtPrimaerenergiebedarfErdgasBwSystemProQm(input),
    co2ErdgasBwSystem: calculateCo2ErdgasBwNormal(input),
    investitionWaermepumpenSystem:
      calculateInvestitionWaermepumpenSystem(input),
    gesamtEnergiekostenWaermepumpenSystem:
      calculateGesamtEnergieKostenWaermepumpenSystem(input),
    gesamtAnnuitaetWaermepumpenSystem:
      calculateGesamtAnnuitaetWaermepumpenSystem(input),
    gesamtPrimaerenergiebedarfWaermepumpenSystemProQm:
      calculateGesamtPrimaerenergiebedarfWaermepumpenSystemProQm(input),
    co2Waermepumpensystem: calculateCo2Waermepumpe(input),
    investitionSwWaermepumpenSystem:
      calculateInvestitionSwWaermepumpenSystem(input),
    gesamtEnergiekostenSwWaermepumpenSystem:
      calculateGesamtEnergieKostenSwWaermepumpenSystem(input),
    gesamtAnnuitaetSwWaermepumpenSystem:
      calculateGesamtAnnuitaetSwWaermepumpenSystem(input),
    gesamtPrimaerenergiebedarfSwWaermepumpenSystemProQm:
      calculateGesamtPrimaerenergiebedarfSwWaermepumpenSystemProQm(input),
    ...
    gesamtEnergiekostenHolzpelletSystem:
      calculateGesamtEnergieKostenHolzpelletSystem(input),
    gesamtAnnuitaetHolzpelletSystem:
      calculateGesamtAnnuitaetHolzpelletSystem(input),
    gesamtPrimaerenergiebedarfHolzpelletSystemProQm:
      calculateGesamtPrimaerenergiebedarfHolzpelletSystemProQm(input),
    co2HolzpelletSystem: calculateCo2Holzpellet(input),
    einsparungPrimaerEnergieOhneAnrechnungGegReferenzgebaeude:
      calculateEinsparungPrimaerEnergieOhneAnrechnungGegReferenzgebaeude(input),
    co2GegReferenzsystem: calculateCo2ErdgasBwGEG(input),
    einsparungCo2AeqEmissionenOhneAnrechnungGegReferenzgebaeude:
      calculateEinsparungCo2AeqEmissionenOhneAnrechnungGegReferenzgebaeude(
        input
      ),
    einsparungPrimaerEnergieMitAnrechnungGegReferenzgebaeude:
      calculateEinsparungPrimaerEnergieMitAnrechnungGegReferenzgebaeude(input),
    einsparungCo2AeqEmissionenMitAnrechnungGegReferenzgebaeude:
      calculateEinsparungCo2AeqEmissionenMitAnrechnungGegReferenzgebaeude(
        input
      ),
    prognoseGegAnforderungenGegReferenzgebaeudePotsdam:
      calculatePrognoseGegAnforderungenGegReferenzgebaeudePotsdam(input),
    prognoseKfwStandardGegReferenzgebaeudePotsdam:
      calculatePrognoseKfwStandardGegReferenzgebaeudePotsdam(input),
    ...
    gesamtPrimaerenergiebedarfGegReferenzgebaeudeProQm:
      calculateGesamtPrimaerenergiebedarfGegReferenzgebaeudeProQm(input),
    Co2Holzpellet: calculateCo2Holzpellet(input),
    Co2Wärmepumpe: calculateCo2Waermepumpe(input),

    //nachträglich hinzugefügt Erdgasbw
    einsparungPrimaerEnergieOhneAnrechnungErdgasBw:
      calculateEinsparungPrimaerEnergieOhneAnrechnungErdgasBw(input),
    einsparungCo2AeqEmissionenOhneAnrechnungErdgasBw:
      calculateEinsparungCo2AeqEmissionenOhneAnrechnungErdgasBw(input),
    einsparungPrimaerEnergieMitAnrechnungErdgasBw:
      calculateEinsparungPrimaerEnergieMitAnrechnungErdgasBw(input),
    einsparungCo2AeqEmissionenMitAnrechnungErdgasBw:
      calculateEinsparungCo2AeqEmissionenMitAnrechnungErdgasBw(input),

    reduzierungGesamtAnnuitaetOhneAnrechnungErdgasBw:
      calculateReduzierungGesamtAnnuitaetOhneAnrechnungErdgasBw(input),
    reduzierungGesamtAnnuitaetMitAnrechnungErdgasBw:
      calculateReduzierungGesamtAnnuitaetMitAnrechnungErdgasBw(input),
    einsparungPrimaerEnergieOhneAnrechnungWaermepumpe:
      calculateEinsparungPrimaerEnergieOhneAnrechnungWaermepumpe(input),
    einsparungCo2AeqEmissionenOhneAnrechnungWaermepumpe:
      calculateEinsparungCo2AeqEmissionenOhneAnrechnungWaermepumpe(input),
    einsparungPrimaerEnergieMitAnrechnungWaermepumpe:
      calculateEinsparungPrimaerEnergieMitAnrechnungWaermepumpe(input),
    einsparungCo2AeqEmissionenMitAnrechnungWaermepumpe:
      calculateEinsparungCo2AeqEmissionenMitAnrechnungWaermepumpe(input),
    reduzierungGesamtAnnuitaetOhneAnrechnungWaermepumpe:
      calculateReduzierungGesamtAnnuitaetOhneAnrechnungWaermepumpe(input),
    reduzierungGesamtAnnuitaetMitAnrechnungWaermepumpe:
      calculateReduzierungGesamtAnnuitaetMitAnrechnungWaermepumpe(input),
    einsparungPrimaerEnergieOhneAnrechnungErdwaermepumpe:
      calculateEinsparungPrimaerEnergieOhneAnrechnungErdwaermepumpe(input),
    einsparungCo2AeqEmissionenOhneAnrechnungErdwaermepumpe:
      calculateEinsparungCo2AeqEmissionenOhneAnrechnungErdwaermepumpe(input),
    einsparungPrimaerEnergieMitAnrechnungErdwaermepumpe:
      calculateEinsparungPrimaerEnergieMitAnrechnungErdwaermepumpe(input),
    einsparungCo2AeqEmissionenMitAnrechnungErdwaermepumpe:
      calculateEinsparungCo2AeqEmissionenMitAnrechnungErdwaermepumpe(input),
    reduzierungGesamtAnnuitaetOhneAnrechnungErdwaermepumpe:
      calculateReduzierungGesamtAnnuitaetOhneAnrechnungErdwaermepumpe(input),
    reduzierungGesamtAnnuitaetMitAnrechnungErdwaermepumpe:
      calculateReduzierungGesamtAnnuitaetMitAnrechnungErdwaermepumpe(input),
    einsparungPrimaerEnergieOhneAnrechnungHolzpellet:
      calculateEinsparungPrimaerEnergieOhneAnrechnungHolzpellet(input),
    einsparungCo2AeqEmissionenOhneAnrechnungHolzpellet:
      calculateEinsparungCo2AeqEmissionenOhneAnrechnungHolzpellet(input),
    einsparungPrimaerEnergieMitAnrechnungHolzpellet:
      calculateEinsparungPrimaerEnergieMitAnrechnungHolzpellet(input),
    einsparungCo2AeqEmissionenMitAnrechnungHolzpellet:
      calculateEinsparungCo2AeqEmissionenMitAnrechnungHolzpellet(input),
    reduzierungGesamtAnnuitaetOhneAnrechnungHolzpellet:
      calculateReduzierungGesamtAnnuitaetOhneAnrechnungHolzpellet(input),
    reduzierungGesamtAnnuitaetMitAnrechnungHolzpellet:
      calculateReduzierungGesamtAnnuitaetMitAnrechnungHolzpellet(input),
  };
}
...
function calculateGesamtPrimaerEnergieBedarfHeizfensterProQm(input) {
  ...
  return (
    calculateGesamtPrimaerEnergieBedarfHeizfenster(aktualisierterInput) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateGesamtPrimaerEnergieBedarfHeizfenster(input) {
  ...
  const netzbezug =
    berechneJaehrlichenNetzbezugHeizfenster(aktualisierterInput);
...
function calculateInvestitionHeizfensterSystem(input) {
  ...
  const gesamtflaecheHeizfenster =
    heizfensterBedarfRechner.calculateMinimaleGesamtflaecheHeizfenster(
      aktualisierterInput.gebaeudeheizlast,
      aktualisierterInput.gebaeudeStandard,
      aktualisierterInput.gebaeudeNutzflaeche
    );

  const anzahlHeizfenster =
    heizfensterBedarfRechner.calculateMinimaleAnzahlHeizfenster(
      aktualisierterInput.gebaeudeheizlast,
      aktualisierterInput.gebaeudeStandard,
      aktualisierterInput.gebaeudeNutzflaeche,
      aktualisierterInput.anzahlBeheizterRaeume,
      aktualisierterInput.groesseHeizfenster
    );

  return kostenRechner.calculateInvestitionskostenHeizfenstersystem(
    gesamtflaecheHeizfenster,
    anzahlHeizfenster,
    aktualisierterInput.installiertePVLeistung,
    aktualisierterInput.nennkapazitaetStromspeicher
  );
}
...
function calculateGesamtStromkostenHeizfenster(input) {
  ...
  return kostenRechner.calculateStromkosten(
    netzbezug,
    aktualisierterInput.strompreisProKWh
  );
}
...
function calculateEinspeiseVerguetungHeizfenster(input) {
  ...
  const gesamtEigenstromnutzung =
    berechneGesamtEigenstromNutzung(aktualisierterInput);

  return einspeiseVerguetungRechner.calculateEinspeiseVerguetung(
    gesamtPvErtrag,
    gesamtEigenstromnutzung,
    aktualisierterInput.einspeiseverguetung
  );
}
...
function calculateGesamtannuitaetHeizfenstersystemOhneAnrechnung(input) {
  ...
  return kostenRechner.calculateGesamtannuitaetHeizfenstersystemOhneAnrechnung(
    investitionskosten,
    stromkosten
  );
}
...
function calculateGesamtannuitaetHeizfenstersystemMitAnrechnung(input) {
  ...
  const investitionskosten =
    calculateInvestitionHeizfensterSystem(aktualisierterInput);

  const stromkosten =
    calculateGesamtStromkostenHeizfenster(aktualisierterInput);

  const pvStromueberschussProJahr =
    berechnePvStromUeberschuss(aktualisierterInput);

  return kostenRechner.calculateGesamtannuitaetHeizfenstersystemMitAnrechnung(
    investitionskosten,
    stromkosten,
    aktualisierterInput.einspeiseverguetung,
...
function calculateCo2Heizfenster(input) {
  const aktualisierterInput = setUpInputWerte(input);
  return co2Emissionenrechner.calculateCO2Strom(
    berechneJaehrlichenNetzbezugHeizfenster(aktualisierterInput)
  );
}
...
function calculateSubstituierteCo2EmissionenHeizfenster(input) {
  ...
  return (
    pvStromueberschussProJahr *
    konstanten.co2ProKWhStrom *
    (aktualisierterInput.anrechnungPvEinspeisung / 100)
  );
}
...
function calculateGesamtPrimaerEnergieGegReferenzgebaeude(aktualisierterInput) {
  const gesamtErdgasbedarf = berechneGesamtErdgasBedarf(
    aktualisierterInput,
    'GEG-Referenzgebäude'
  );

  return (
    gesamtErdgasbedarf * konstanten.primaerenergiefaktorGas +
    strombedarfsRechner.calculateStrombedarfHaushalt(
      aktualisierterInput.bewohnerAnzahl
    ) *
      konstanten.primaerEnergieFaktorStrom
  );
}
...
function calculateCo2ErdgasBwGEG(input) {
  ...
  const gesamtStrombedarfHaushalt =
    strombedarfsRechner.calculateStrombedarfHaushalt(
      aktualisierterInput.bewohnerAnzahl
    );

  return (
    co2Emissionenrechner.calculateCO2Strom(gesamtStrombedarfHaushalt) +
    co2Emissionenrechner.calculateCO2Erdgas(gesamtErdgasbedarf)
  );
}
...
function calculateInvestitionErdgasBwSystem(input) {
  ...
  return kostenRechner.calculateInvestitionskostenErdgasBWSystem(
    aktualisierterInput.anzahlBeheizterRaeume
  );
}
...
function calculateGesamtEnergiekostenErdgasBwSystem(input) {
  ...
  const erdgasbedarf = berechneGesamtErdgasBedarf(
    aktualisierterInput,
    aktualisierterInput.gebaeudeStandard
  );

  const erdgaskosten = kostenRechner.calculateErdgasKosten(
    erdgasbedarf,
    aktualisierterInput.erdgaspreis
  );

  const haushaltsstrombedarf = strombedarfsRechner.calculateStrombedarfHaushalt(
    aktualisierterInput.bewohnerAnzahl
  );

  const stromkosten = kostenRechner.calculateStromkosten(
...
function calculateGesamtannuitaetErdgasBwSystem(input) {
  ...
  return kostenRechner.calculateGesamtannuitaetErdgasBW(
    aktualisierterInput.anzahlBeheizterRaeume,
    erdgaskosten,
    stromkosten
  );
...
function calculateGesamtPrimaerenergiebedarfErdgasBwSystemProQm(input) {
  ...
  const haushaltsstrombedarf =
    strombedarfsRechner.calculateStrombedarfHaushalt(
      aktualisierterInput.bewohnerAnzahl
    ) / aktualisierterInput.gebaeudeNutzflaeche;
  const erdgasbedarf =
    berechneGesamtErdgasBedarf(
      aktualisierterInput,
      aktualisierterInput.gebaeudeStandard
    ) / aktualisierterInput.gebaeudeNutzflaeche;

  return (
    haushaltsstrombedarf * konstanten.primaerEnergieFaktorStrom +
    erdgasbedarf * konstanten.primaerenergiefaktorGas
  );
}
...
function calculateGesamtPrimaerEnergiebedarfWaermepumpe(aktualisierterInput) {
  const energieBedarfHeizenTww = berechneEnergiebedarfHeizenUndTww(
    aktualisierterInput,
    konstanten.twwwpfaktor
  );

  return (
    energieBedarfHeizenTww * konstanten.primaerEnergieFaktorStrom +
    strombedarfsRechner.calculateStrombedarfHaushalt(
      aktualisierterInput.bewohnerAnzahl
    ) *
      konstanten.primaerEnergieFaktorStrom
...
function calculateInvestitionWaermepumpenSystem(input) {
  ...
  return kostenRechner.calculateInvestitionskostenWaermepumpenSystem(
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateGesamtEnergieKostenWaermepumpenSystem(input) {
  ...
  const twwBedarf = twwBedarfRechner.calculateTwwBedarf(
    aktualisierterInput.gebaeudeNutzflaeche
  );

  const heizwaermebedarf = heizwaermebedarfRechner.calculateHeizwaermebedarf(
    aktualisierterInput.gebaeudeStandard,
    aktualisierterInput.gebaeudeNutzflaeche
  );
  ...
  return kostenRechner.calculateGesamtEnergiekostenWaermepumpenSystem(
    twwBedarf,
    heizwaermebedarf,
    haushaltsstrombedarf,
    aktualisierterInput.strompreisProKWh
...
function calculateGesamtAnnuitaetWaermepumpenSystem(input) {
  ...
  return kostenRechner.calculateGesamtAnnuitaetWaermepumpenSystem(
    aktualisierterInput.gebaeudeNutzflaeche,
    heizwaermebedarf,
    twwBedarf,
    haushaltsstrombedarf,
...
function calculateGesamtPrimaerenergiebedarfWaermepumpenSystemProQm(input) {
  const aktualisierterInput = setUpInputWerte(input);
  return (
    calculateGesamtPrimaerEnergiebedarfWaermepumpe(input) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateCo2Waermepumpe(input) {
  ...
  const strombedarfHaushalt = strombedarfsRechner.calculateStrombedarfHaushalt(
    aktualisierterInput.bewohnerAnzahl
  );

  const gesamtStrombedarf =
    berechneEnergiebedarfHeizenUndTww(
      aktualisierterInput,
      konstanten.twwwpfaktor
    ) + strombedarfHaushalt;
...
//TODO: zur Api hinzufügen
function calculateGesamtPrimaerEnergiebedarfErdwaermePumpe(
  aktualisierterInput
) {
  const energieBedarfHeizenTww = berechneEnergiebedarfHeizenUndTww(
    aktualisierterInput,
    konstanten.twwewfaktor
  );
...
function calculateInvestitionSwWaermepumpenSystem(input) {
  ...
  return kostenRechner.calculateInvestitionskostenSwWaermepumpenSystem(
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateGesamtEnergieKostenSwWaermepumpenSystem(input) {
  ...
  return kostenRechner.calculateGesamtEnergieKostenSwWaermepumpenSystem(
    heizwaermebedarf,
    twwBedarf,
    haushaltsstrombedarf,
    aktualisierterInput.strompreisProKWh
...
function calculateGesamtAnnuitaetSwWaermepumpenSystem(input) {
  ...
  return kostenRechner.calculateGesamtAnnuitaetSwWaermepumpenSystem(
    aktualisierterInput.gebaeudeNutzflaeche,
    heizwaermebedarf,
    twwBedarf,
    haushaltsstrombedarf,
...
function calculateGesamtPrimaerenergiebedarfSwWaermepumpenSystemProQm(input) {
  const aktualisierterInput = setUpInputWerte(input);
  return (
    calculateGesamtPrimaerEnergiebedarfErdwaermePumpe(aktualisierterInput) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateGesamtPrimaerEnergiebedarfHolzpellets(aktualisierterInput) {
  const energieBedarfHeizenTww = berechneEnergiebedarfHeizenUndTww(
    aktualisierterInput,
    konstanten.aufwandzahlholzpellets
  );

  return (
    energieBedarfHeizenTww * konstanten.primaerEnergieFaktorHolzpellets +
    strombedarfsRechner.calculateStrombedarfHaushalt(
      aktualisierterInput.bewohnerAnzahl
    ) *
      konstanten.primaerEnergieFaktorStrom
...
function calculateInvestitionHolzpelletSystem(input) {
  ...
  return kostenRechner.calculateInvestitionskostenHolzpelletSystem(
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateGesamtEnergieKostenHolzpelletSystem(input) {
  ...
  return kostenRechner.calculateGesamtEnergiekostenHolzpelletSystem(
    heizwaermebedarf,
    twwBedarf,
    haushaltsstrombedarf,
    aktualisierterInput.strompreisProKWh
...
function calculateGesamtAnnuitaetHolzpelletSystem(input) {
  ...
  return kostenRechner.calculateGesamtannuitaetHolzpelletSystem(
    aktualisierterInput.gebaeudeNutzflaeche,
    heizwaermebedarf,
    twwBedarf,
    haushaltsstrombedarf,
...
function calculateGesamtPrimaerenergiebedarfHolzpelletSystemProQm(input) {
  const aktualisierterInput = setUpInputWerte(input);
  return (
    calculateGesamtPrimaerEnergiebedarfHolzpellets(aktualisierterInput) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateCo2Holzpellet(input) {
  ...
  const energiebedarfHeizenUndTww = berechneEnergiebedarfHeizenUndTww(
    aktualisierterInput,
    konstanten.aufwandzahlholzpellets
  );
  ...
  return (
    co2Emissionenrechner.calculateCO2Strom(strombedarfHaushalt) +
    co2Emissionenrechner.calculateCO2Holzpellet(energiebedarfHeizenUndTww)
  );
}
...
function calculateGesamtPrimaerenergiebedarfGegReferenzgebaeudeProQm(input) {
  const aktualisierterInput = setUpInputWerte(input);
  const gesamtPrimaerEnergieGegReferenzgebaeude =
    calculateGesamtPrimaerEnergieGegReferenzgebaeude(aktualisierterInput);
  return (
    gesamtPrimaerEnergieGegReferenzgebaeude /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}

function calculateEinsparungPrimaerEnergieOhneAnrechnungGegReferenzgebaeude(
  input
) {
  const aktualisierterInput = setUpInputWerte(input);

  const gesamtPrimaerEnergieHeizfenster =
    calculateGesamtPrimaerEnergieBedarfHeizfenster(aktualisierterInput);
  ...
  return (
    (gesamtPrimaerEnergieGegReferenzgebaeude -
      gesamtPrimaerEnergieHeizfenster) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}

function calculateEinsparungPrimaerEnergieMitAnrechnungGegReferenzgebaeude(
  input
) {
  const aktualisierterInput = setUpInputWerte(input);
  ...
  const substituiertePrimaerEnergie =
    calculateSubstituiertePrimaerEnergieHeizfenster(aktualisierterInput);
  ...
  return (
    (gesamtPrimaerEnergieGegReferenzgebaeude -
      (gesamtPrimaerEnergieHeizfenster - substituiertePrimaerEnergie)) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}

function calculateEinsparungCo2AeqEmissionenMitAnrechnungGegReferenzgebaeude(
  input
) {
  const aktualisierterInput = setUpInputWerte(input);

  const gesamtCo2Heizfenstersystem =
    calculateCo2Heizfenster(aktualisierterInput);

  const gesamtCo2ErdgasBwSystem = calculateCo2ErdgasBwGEG(aktualisierterInput);

  const gesamtSubstuierteCo2Emissionen =
    calculateCo2HeizfensterAnrechnung(aktualisierterInput);

  return (
    gesamtCo2ErdgasBwSystem -
    (gesamtCo2Heizfenstersystem - gesamtSubstuierteCo2Emissionen)
  );
}

function calculateEinsparungCo2AeqEmissionenOhneAnrechnungGegReferenzgebaeude(
  input
) {
  const aktualisierterInput = setUpInputWerte(input);
...
function calculatePrognoseGegAnforderungenGegReferenzgebaeudePotsdam(input) {
  ...
  let gesamtPrimaerEnergiebedarfEinsparrecht =
    berechnePrimaerEnergiebedarfNormativeVergleichsrechnung(
      aktualisierterInput
    );
  let gesamtPrimaerEnergiebedarfGEGReferenz =
    calculateGesamtPrimaerEnergieGegReferenzgebaeude(aktualisierterInput);
  let gesamtEinsparungPrimaerEnergiebedarf =
    gesamtPrimaerEnergiebedarfGEGReferenz -
    gesamtPrimaerEnergiebedarfEinsparrecht;
...
function calculatePrognoseKfwStandardGegReferenzgebaeudePotsdam(input) {
  ...
  if (aktualisierterInput.gebaeudeStandard == 'KfW 55') {
    if (
      gesamtEinsparungPrimaerEnergiebedarf <=
      gesamtPrimaerEnergiebedarfGEGReferenz * 0.55
    ) {
      return true;
    } else {
      return false;
  ...
  if (aktualisierterInput.gebaeudeStandard == 'KfW 40') {
    if (
      gesamtEinsparungPrimaerEnergiebedarf <=
      gesamtPrimaerEnergiebedarfGEGReferenz * 0.4
    ) {
      return true;
    } else {
      return false;
...
function calculateEinsparungPrimaerEnergieOhneAnrechnungErdgasBw(input) {
  ...
  const gesamtPrimaerEnergieErdgasBw =
    berechneGesamtErdgasBedarf(
      aktualisierterInput,
      aktualisierterInput.gebaeudeStandard
    ) *
      konstanten.primaerenergiefaktorGas +
    strombedarfsRechner.calculateStrombedarfHaushalt(
      aktualisierterInput.bewohnerAnzahl
    ) *
      konstanten.primaerEnergieFaktorStrom;
  ...
  return (
    (gesamtPrimaerEnergieErdgasBw - gesamtPrimaerEnergieHeizfenster) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateEinsparungCo2AeqEmissionenOhneAnrechnungErdgasBw(input) {
  ...
  const gesamtCo2ErdgasBwSystem =
    calculateCo2ErdgasBwNormal(aktualisierterInput);
...
function calculateReduzierungGesamtAnnuitaetOhneAnrechnungErdgasBw(input) {
  ...
  const gesamtannuitaetHeizfenster =
    calculateGesamtannuitaetHeizfenstersystemOhneAnrechnung(input);
...
function calculateEinsparungPrimaerEnergieOhneAnrechnungWaermepumpe(input) {
  ...
  const gesamtPrimaerenergieBedarfWaermepumpe =
    calculateGesamtPrimaerEnergiebedarfWaermepumpe(aktualisierterInput);

  const gesamtPrimaerenergieBedarfHeizfenster =
    calculateGesamtPrimaerEnergieBedarfHeizfenster(aktualisierterInput);

  return (
    (gesamtPrimaerenergieBedarfWaermepumpe -
      gesamtPrimaerenergieBedarfHeizfenster) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateEinsparungPrimaerEnergieMitAnrechnungWaermepumpe(input) {
  ...
  return (
    (gesamtPrimaerenergieBedarfWaermepumpe -
      (gesamtPrimaerenergieBedarfHeizfenster - substituiertePrimaerEnergie)) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateEinsparungCo2AeqEmissionenMitAnrechnungWaermepumpe(input) {
  ...
  return (
    gesamtCo2Waermepumpe -
    (gesamtCo2Heizfenstersystem - gesamtSubstuierteCo2Emissionen)
  );
}

function calculateReduzierungGesamtAnnuitaetOhneAnrechnungWaermepumpe(input) {
  const gesamtannuitaetWaermepumpe =
    calculateGesamtAnnuitaetWaermepumpenSystem(input);

  const gesamtannuitaetHeizfenstersystem =
    calculateGesamtannuitaetHeizfenstersystemOhneAnrechnung(input);
...
function calculateEinsparungPrimaerEnergieOhneAnrechnungErdwaermepumpe(input) {
  ...
  const gesamtPrimaerEnergiebedarfErdwaermePumpe =
    calculateGesamtPrimaerEnergiebedarfErdwaermePumpe(
      aktualisierterInput,
      input
    );
  ...
  return (
    (gesamtPrimaerEnergiebedarfErdwaermePumpe -
      gesamtPrimaerEnergieBedarfHeizfenster) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}

function calculateEinsparungCo2AeqEmissionenOhneAnrechnungErdwaermepumpe(
  input
) {
  const aktualisierterInput = setUpInputWerte(input);
  ...
  const gesamtCo2Erdwaermepumpe =
    calculateCo2Erdwaermepumpe(aktualisierterInput);
...
function calculateEinsparungCo2AeqEmissionenMitAnrechnungErdwaermepumpe(input) {
  ...
  return (
    gesamtCo2Erdwaermepumpe -
    (gesamtCo2Heizfenstersystem - gesamtSubstuierteCo2Emissionen)
  );
}

function calculateReduzierungGesamtAnnuitaetOhneAnrechnungErdwaermepumpe(
  input
) {
  const gesamtannuitaetErdwaermepumpensystem =
    calculateGesamtAnnuitaetSwWaermepumpenSystem(input);
  ...
  return (
    gesamtannuitaetErdwaermepumpensystem - gesamtannuitaetHeizfenstersystem
  );
}
...
function calculateEinsparungPrimaerEnergieOhneAnrechnungHolzpellet(input) {
  ...
  const gesamtPrimaerEnergiebedarfHolzpellet =
    calculateGesamtPrimaerEnergiebedarfHolzpellets(aktualisierterInput);
  ...
  return (
    (gesamtPrimaerEnergiebedarfHolzpellet -
      gesamtPrimaerEnergieBedarfHeizfenster) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateEinsparungPrimaerEnergieMitAnrechnungHolzpellet(input) {
  ...
  const gesamtPrimaerenergieBedarfHolzpellets =
    calculateGesamtPrimaerEnergiebedarfHolzpellets(aktualisierterInput);
  ...
  return (
    (gesamtPrimaerenergieBedarfHolzpellets -
      (gesamtPrimaerenergieBedarfHeizfenster - substituiertePrimaerEnergie)) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function setUpInputWerte(input) {
  ...
  if (!('installiertePVLeistung' in defaultValuesKopie)) {
    defaultValuesKopie['installiertePVLeistung'] =
      installiertePvLeistungRechner.calculateInstalliertePVLeistung(
        defaultValuesKopie.gebaeudeNutzflaeche,
        defaultValuesKopie.etagenAnzahl
      );
  }
  ...
  if (!('einspeiseverguetung' in defaultValuesKopie)) {
    defaultValuesKopie['einspeiseverguetung'] =
      defaultValuesKopie.installiertePVLeistung <= 10
        ? konstanten.einspeiseverguetungBis10kwp
        : konstanten.einspeiseverguetungAb10kwp;
  }
  ...
  if (!('pvErtragNutzereingabeArray' in defaultValuesKopie)) {
    defaultValuesKopie['pvErtragGesamt'] = pvErtragsRechner.calculatePvErtrag(
      defaultValuesKopie.bundesland,
      defaultValuesKopie.installiertePVLeistung
    );
  }
  //Anderenfalls addiere alle Werte des Nutzereingabe Arrays
  else {
    defaultValuesKopie['pvErtragGesamt'] =
      pvErtragsRechner.calculatePvErtragNutzereingabe(
        defaultValuesKopie['pvErtragNutzereingabeArray']
      );
  }
...
function berechneJaehrlichenNetzbezugHeizfenster(aktualisierterInput) {
  const gesamtStrombedarf = strombedarfsRechner.calculateGesamtStrombedarf(
    aktualisierterInput.heizfensterTyp,
    aktualisierterInput.gebaeudeStandard,
    aktualisierterInput.gebaeudeNutzflaeche,
    aktualisierterInput.bewohnerAnzahl
  );
  ...
  return netzbezugRechner.calculateNetzbezugJaehrlich(
    gesamtStrombedarf,
    gesamtEigenstromNutzung
  );
}
...
function berechneGesamtErdgasBedarf(aktualisierterInput, gebaeudeStandard) {
  ...
  const heizwaermebedarfProQMProMonat =
    heizwaermebedarfRechner.calculateHeizwaermebedarfProQm(
      gebaeudeStandard,
      aktualisierterInput.eingabeHeizwaermebedarfProQM
    );
  ...
  return erdgasRechner.calculateErdgasbedarf(
    gesamtEnergiebedarf,
    aktualisierterInput.gebaeudeNutzflaeche,
...
function berechneEnergiebedarfHeizenUndTww(aktualisierterInput, aufwandsZahl) {
  ...
  const trinkwasserBedarfProQm = twwBedarfRechner.calculateTwwbedarfProQm(
    aktualisierterInput.gebaeudeNutzflaeche
  );
  ...
  return (
    (strombedarfHeizenProQm + strombedarfTwwProQm) *
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateSubstituiertePrimaerEnergieHeizfensterProQM(input) {
  const aktualisierterInput = setUpInputWerte(input);
  return (
    calculateSubstituiertePrimaerEnergieHeizfenster(input) /
    aktualisierterInput.gebaeudeNutzflaeche
  );
}
...
function calculateCo2HeizfensterAnrechnung(input) {
  const aktualisierterInput = setUpInputWerte(input);
  const gesamtPvStromueberschuss =
    berechnePvStromUeberschuss(aktualisierterInput);
  return co2Emissionenrechner.calculateCO2Strom(gesamtPvStromueberschuss);
}

//Benötigt för Prognose Berechnungen
function berechnePrimaerEnergiebedarfNormativeVergleichsrechnung(
  aktualisierterInput
) {
  const gesamtStrombedarfHeizfenstersystem =
    strombedarfsRechner.calculateGesamtStrombedarf(
      aktualisierterInput.heizfensterTyp,
      aktualisierterInput.gebaeudeStandard,
      aktualisierterInput.gebaeudeNutzflaeche,
      aktualisierterInput.bewohnerAnzahl
  ...
  const P_pkNormativeVergleichsrechnung =
    0.9 * aktualisierterInput.installiertePVLeistung;
  const C_effNormativeVergleichsrechnung =
    konstanten.normativevergleichsrechnungN_DoD *
    aktualisierterInput.nennkapazitaetStromspeicher;
  const v_QNormativeVergleichsrechnung =
    (P_pkNormativeVergleichsrechnung / gesamtStrombedarfHeizfenstersystem) *
    1000;
  const c_QNormativeVergleichsrechnung =
    (C_effNormativeVergleichsrechnung / gesamtStrombedarfHeizfenstersystem) *
    1000;
  const f_DLENormativeVergleichsrechnung =
    0.8 * ((18 - C_effNormativeVergleichsrechnung / 2) / 18);
  const f_BattNormativeVergleichsrechnung = Math.min(
    0.9 *
      ((gesamtStrombedarfHeizfenstersystem -
        f_DLENormativeVergleichsrechnung *
          strombedarfsRechner.calculateStrombedarfTwwProQm(
            aktualisierterInput.gebaeudeNutzflaeche
          ) *
          aktualisierterInput.gebaeudeNutzflaeche) /
        berechneKonstantenormativevergleichsrechnungDirekteEigenstromnutzung(
          aktualisierterInput
        )),
    konstanten.normativevergleichsrechnungPVErtrag /
      berechneKonstantenormativevergleichsrechnungDirekteEigenstromnutzung(
        aktualisierterInput
      ),
    Math.max(
      1,
      (0.2 * Math.log(v_QNormativeVergleichsrechnung) + 1.85) *
        c_QNormativeVergleichsrechnung **
          (0.1 * Math.log(v_QNormativeVergleichsrechnung) + 0.25)
    )
  );
  const gesamtEigenstromNutzungNormativeVergleichsrechnung =
    f_BattNormativeVergleichsrechnung *
    berechneKonstantenormativevergleichsrechnungDirekteEigenstromnutzung(
      aktualisierterInput.gebaeudeStandard
    ); //Switch case einbauen für lonstante
  const batterieverlusteNormativeVergleichsrechnung =
    berechneKonstantenormativevergleichsrechnungDirekteEigenstromnutzung(
      aktualisierterInput.gebaeudeStandard
    ) *
    (1 - konstanten.normativevergleichsrechnungN_Batt) *
    (f_BattNormativeVergleichsrechnung - 1);
  const netzbezugNormativeVergleichsrechnung =
    gesamtStrombedarfHeizfenstersystem -
    gesamtEigenstromNutzungNormativeVergleichsrechnung +
    batterieverlusteNormativeVergleichsrechnung;
  const gesamtPrimaerEnergieBedarfNomrativeVergleichsrechnung =
    netzbezugNormativeVergleichsrechnung * konstanten.primaerEnergieFaktorStrom;
  return gesamtPrimaerEnergieBedarfNomrativeVergleichsrechnung;
}

function berechneKonstantenormativevergleichsrechnungDirekteEigenstromnutzung(
  gebaeudeStandard
) {

...
function calculateDirekteEigenstromNutzungJaehrlich(
  gesamtStrombedarf,
  gesamtPvErtrag
) {
  if (
    ...
    return calculateMengeDirekteEigenstromNutzungAusAnteil(
      gesamtPvErtrag,
      gesamtStrombedarf,
      calculateAnteilAnStromverbrauch(gesamtStrombedarf, gesamtPvErtrag)
    );
  }
  return 0;
}

function calculateDirekteEigenstromNutzungMonatlich(
  direkteEigenstromNutzung,
  desn
) {
  let direkteEigentstromNutzungMonatlich = [];
  for (let i = 0; i < 12; i++) {
    direkteEigentstromNutzungMonatlich[i] =
      direkteEigenstromNutzung * (desn[i] / 100);
  }
  return direkteEigentstromNutzungMonatlich;
}

function calculateVorbedingungenDirekteEigenstromNutzungJaehrlich(
  gesamtStrombedarf,
  gesamtPvErtrag
) {
...
function calculateMengeDirekteEigenstromNutzungAusAnteil(
  pvStromErtrag,
  gesamtStrombedarf,
  anteilDirektNutzung
) {
...
function calculateAnteilAnStromverbrauch(gesamtStromVerbrauch, pvErtrag) {
  ...
  let zeilennummer = findNextSmallerValue(
    gesamtStromVerbrauch,
    stromVerbrauchGrenzen
  );
  let indexSpaltennummer = ermittleNeuenIndex(spaltennummer, 9);
  let indexZeilennummer = ermittleNeuenIndex(zeilennummer, 12);
  let kleinererErtragKleinererVerbrauch =
    anteileTabelle[indexZeilennummer][indexSpaltennummer];
  let kleinererErtragGroessererVerbrauch =
    anteileTabelle[indexZeilennummer + 1][indexSpaltennummer];
  let groessererErtragKleinererVerbrauch =
    anteileTabelle[indexZeilennummer][indexSpaltennummer + 1];
  let groessererErtragGroessererVerbrauch =
    anteileTabelle[indexZeilennummer + 1][indexSpaltennummer + 1];
  ...
  let abschaetzungErtragKleinerVerbauch =
    kleinererErtragKleinererVerbrauch +
    ((groessererErtragKleinererVerbrauch - kleinererErtragKleinererVerbrauch) /
      (naechstGroessererErtrag - naechstKleinererErtrag)) *
      (pvErtrag - naechstKleinererErtrag);
  let abschaetzungErtragGrosserVerbrauch =
    kleinererErtragGroessererVerbrauch +
    ((groessererErtragGroessererVerbrauch -
      kleinererErtragGroessererVerbrauch) /
      (naechstGroessererErtrag - naechstKleinererErtrag)) *
      (pvErtrag - naechstKleinererErtrag);
  return (
    abschaetzungErtragKleinerVerbauch +
    ((abschaetzungErtragGrosserVerbrauch - abschaetzungErtragKleinerVerbauch) /
      (naechstGroessererVerbrauch - naechstKleinererVerbrauch)) *
      (gesamtStromVerbrauch - naechstKleinererVerbrauch)
  );
}
